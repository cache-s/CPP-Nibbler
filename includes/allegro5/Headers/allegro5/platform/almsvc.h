...
#define AL_FUNC(type, name, args)      _allegro5_DLL type __cdecl name args
#define AL_METHOD(type, name, args)    type (__cdecl *name) args
#define AL_FUNCPTR(type, name, args)   extern _allegro5_DLL type (__cdecl *name) args

#ifdef AL_INLINE
   #define END_allegro5_INLINE(name)         void *_force_instantiate_##name = name;
#else
...
#define AL_INLINE(type, name, args, code)        __inline _allegro5_DLL type __cdecl name args code END_allegro5_INLINE(name)
#define AL_INLINE_STATIC(type, name, args, code) __inline type __cdecl name args code END_allegro5_INLINE(name)
...
/* __func__ is C99 */
   ...
   /* MSVC versions before VC7 don't have __FUNCTION__ */
      ...
      #define __func__ "???"
   #else
      #define __func_allegro5__FUNCTION__
   #endif
#endif
